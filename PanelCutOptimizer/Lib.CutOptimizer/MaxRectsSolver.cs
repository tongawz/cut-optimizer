...
namespace LIB.CutOptimizer
{
  public class MaxRectsSolver
  {
    ...
    private readonly decimal _baseAreaM2;
    ...
    private (int, List<PositionedPanel>)? TryPackInParallelWithPruning(List<List<Panel>> combinations)
    {
      ...
      while (combinations.Count != 0)
      {
        ...
        var newBestResult = (area: 0, positioned: new List<PositionedPanel>());
    ...
    private static int CalculateArea(List<Panel> comb)
    {
    ...
    private List<List<Panel>> GetPanelCombinations(List<Panel> panels)
    {
      ...
      if (_rotate)
      {
        ...
        plusRotatedPanels = [.. panels, .. rotatedPanels];
      }

      plusRotatedPanels = [.. plusRotatedPanels.OrderByDescending(x => x.AreaM2)];
      ...
      Parallel.For(1, maxMask, new ParallelOptions { MaxDegreeOfParallelism = threadsToUse }, mask =>
      {
        ...
        for (int i = 0; i < n; i++)
        {
          if ((mask & 1 << i) != 0)
          {
            ...
            if (!usedNames.Add(panel.PanelName ?? string.Empty)) continue;
